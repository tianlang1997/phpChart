<?php
	...
	*/
	...
	if($_GET['bar_Chart'] && $_SESSION[$_GET['bar_Chart']] && $_SESSION[$_GET['bar_Chart']]["dataArr"])
	{
		drawPicture($_SESSION[$_GET['bar_Chart']]);
	}
	...
	function drawPicture($chartData){	
		...
		for($i=1;$i<count($data) && $i<count($dataIndex);$i++){
			imagefilledrectangle($imgrs,$start_X+$i*$XcoordWidth-$XcoordWidth/3 , turnX($start_Y), $start_X+$i*$XcoordWidth+$XcoordWidth/3 , turnX($start_Y+$data[$i-1]*$scaleRate) , $penColor);	
			imagettftext($imgrs,$fontSize,0,$start_X+(($i+1)*$XcoordWidth),turnX($start_Y+$data[$i]*$scaleRate+20),$penColor,$font,$data[$i]);	
		}
		imagefilledrectangle($imgrs,$start_X+count($data)*$XcoordWidth-$XcoordWidth/3 , turnX($start_Y), $start_X+count($data)*$XcoordWidth+$XcoordWidth/3 , turnX($start_Y+$data[count($data)-1]*$scaleRate) , $penColor);
		//输出图像  
